...
class GradientSobelFilter:
    def __init__(self, n_groups=1):
        ...
        self.sfx.weight.data = torch.Tensor([[[[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]]]]).expand([self.n_groups,1,1,1])
        self.sfx.to('cuda')

        self.sfy = nn.Conv2d(self.n_groups, self.n_groups, 3, 1, 1, bias=False, groups=self.n_groups)
        self.sfy.weight.data = torch.Tensor([[[[-1, -2, -1], [0, 0, 0], [1, 2, 1]]]]).expand([self.n_groups,1,1,1])
        self.sfy.to('cuda')
