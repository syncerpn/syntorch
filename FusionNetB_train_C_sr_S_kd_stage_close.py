...
def train_kd_stage_close(epoch, optim, target_stage):
    perfs = []
    ...
    for batch_idx, (x, yt) in tqdm.tqdm(enumerate(XYtrain), total=len(XYtrain)):
        ...
        fea_teacher, fea_student, merge_map = core.forward_stage_wise_close_mask(x, gsf, args.psi, target_stage)

        loss_func = loss.create_loss_func(args.loss)
        batch_loss = loss_func(fea_student * (1.0 - merge_map), fea_teacher * (1.0 - merge_map))
...
for target_stage in range(core.ns):
    ...
    for epoch in range(args.start_epoch, args.max_epochs+1):
        ...
        train_kd_stage_close(epoch, optim_phase_2, target_stage)